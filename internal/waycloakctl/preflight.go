...
func Preflight(ctx context.Context, clients *Clients, overlayCIDR string) (PreflightReport, error) {
	report := PreflightReport{APIVersion: OutputAPIVersion, Kind: "PreflightReport", Compatible: true, Profile: "networking.waycloak.io/Core-v1", Networking: NetworkSummary{OverlayCIDR: overlayCIDR}}
	...
	report.add("kubernetes-version", minorErr == nil && version.Major == "1" && minor >= 36, "Kubernetes "+version.GitVersion+" is supported", "Use a published Kubernetes 1.36+ support-matrix row")
	...
	report.add("nodes", nodesSupported, "All nodes report a supported Linux kernel and architecture", "Use Linux amd64/arm64 nodes with kernel 5.10+ and a published runtime row")
	...
	report.add("chained-cni", report.CNI.Name != "", "A published chained-CNI layout was detected", "Use the explicit advanced path or a published Kind/k3s-Flannel support row")
...
func detectCNI(items []appsv1.DaemonSet, kubernetesVersion string) CNISummary {
	...
	for _, item := range items {
		for _, container := range append(item.Spec.Template.Spec.InitContainers, item.Spec.Template.Spec.Containers...) {
			...
			case strings.Contains(identity, "kindnet"):
				return CNISummary{Name: "kindnet", ConfigPath: "/etc/cni/net.d/10-kindnet.conflist", BinaryPath: "/opt/cni/bin/waycloak-cni"}
			case strings.Contains(identity, "flannel"):
				return CNISummary{Name: "flannel", ConfigPath: "/etc/cni/net.d/10-flannel.conflist", BinaryPath: "/opt/cni/bin/waycloak-cni"}
	...
	if strings.Contains(strings.ToLower(kubernetesVersion), "k3s") && !alternate {
		return CNISummary{Name: "k3s-flannel", ConfigPath: "/var/lib/rancher/k3s/agent/etc/cni/net.d/10-flannel.conflist", BinaryPath: "/var/lib/rancher/k3s/data/cni/waycloak-cni"}
	}
	return CNISummary{}
}
