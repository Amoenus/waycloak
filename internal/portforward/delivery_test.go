...
func TestDeliveryManagerWithdrawsOnlyExactReferencedAdapter(t *testing.T) {
	...
	adapter := &fakeAdapterProtocol{now: now}
	delivery := DeliveryManager{Adapter: adapter, Now: func() time.Time { return now }}
	intent := WithdrawalIntent{APIVersion: RuntimeAPIVersion, LeaseNamespace: "apps", LeaseUID: "lease-uid", GatewayUID: "gateway-uid", HandoffGeneration: 3, PodUID: "pod-uid", AdapterName: "qbittorrent", ApplicationEndpointRetired: true}
	want := AdapterWithdrawalIntent{APIVersion: AdapterAPIVersion, LeaseNamespace: intent.LeaseNamespace, LeaseUID: intent.LeaseUID,
		HandoffGeneration: intent.HandoffGeneration, PodUID: intent.PodUID, ApplicationEndpointRetired: true}
	if withdrawn, err := delivery.Withdraw(context.Background(), intent); err != nil || !withdrawn || !reflect.DeepEqual(adapter.withdrawn, want) {
...
type fakeAdapterProtocol struct {
	...
	withdrawn AdapterWithdrawalIntent
	stale     bool
}
...
func (a *fakeAdapterProtocol) Withdraw(_ context.Context, name wayv1.ObjectName, intent AdapterWithdrawalIntent) (bool, error) {
	if name == "" {
