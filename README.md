...
The Core security boundary is a mandatory chained CNI plugin backed by a privileged per-node agent. It installs deny-first state before CNI `ADD` succeeds. Application Pods receive no Waycloak sidecar or init container, Linux capability, host mount, VPN credential, or Kubernetes credential. A missing, rejected, unauthorized, unhealthy, or unprogrammed route fails closed; it never silently selects ordinary egress.
