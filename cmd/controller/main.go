...
func main() {
	var metricsAddr, probeAddr, agentImage string
	var leader bool
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{Scheme: scheme, Metrics: metricsserver.Options{BindAddress: metricsAddr}, HealthProbeBindAddress: probeAddr, LeaderElection: leader, LeaderElectionID: "waycloak-controller.networking.waycloak.io"})
	if err != nil {
	...
	if err = (&waycontroller.WorkloadGCReconciler{Client: mgr.GetClient(), Quarantine: 5 * time.Minute}).SetupWithManager(mgr); err != nil {
