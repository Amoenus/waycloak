...
The Core packet matrix includes TCP, UDP, DNS over UDP and TCP, and fragmentation. Ordinary egress is permitted only for Pods that were never enrolled. See [ADR 0034](../decisions/0034-cni-creation-time-enforcement.md) and the [CNI feasibility evidence](../implementation/cni-creation-time-feasibility.md).
