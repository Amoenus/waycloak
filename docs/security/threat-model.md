...
| Mount VPN credential | gateway engine | referenced Secret mounted read-only only into engine; engine drops all capabilities and regains only `NET_ADMIN`, `CHOWN`, `DAC_OVERRIDE`, and `SETUID` for Gluetun's generated UID-1000 OpenVPN configuration and explicit OpenVPN privilege drop | gateway refs unresolved/not ready | Secret rotation restarts/reloads engine per class contract | agent/application mount absence; exact engine capability set; UID/mode read-rewrite and privilege drop; class/status redaction |
| Serve protected gateway DNS | gateway agent + Gluetun | workload DNS is redirected to the gateway overlay on TCP/UDP 1053; the credential-free gateway agent forwards only to Gluetun's loopback TCP/UDP 53 server, which exclusively owns port 53 in the shared network namespace | DNS and gateway Ready false; enrolled traffic remains denied | level-based listener/rule repair after tunnel DNS observation returns | bind collision, UDP/TCP forwarding, tunnel/DNS loss, no ordinary resolver fallback |
