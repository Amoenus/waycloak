...
| Mount VPN credential | gateway engine | referenced Secret mounted read-only only into engine; engine drops all capabilities and regains only `NET_ADMIN`, `CHOWN`, and `DAC_OVERRIDE` for Gluetun's generated UID-1000 OpenVPN configuration | gateway refs unresolved/not ready | Secret rotation restarts/reloads engine per class contract | agent/application mount absence; exact engine capability set; UID/mode read-rewrite; class/status redaction |
