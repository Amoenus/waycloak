...
| Resolve enrollment/binding | node agent | read-only Pod and binding cache; exact UID and assigned node | missing/rejected/unobserved remains denied | object/watch recovery re-evaluates exact UID | label removal, admission loss, UID/name reuse, hostile binding |
