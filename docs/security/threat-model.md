...
| Mount VPN credential | gateway engine | referenced Secret mounted read-only only into engine; engine drops all capabilities and regains only `NET_ADMIN`, `CHOWN`, `DAC_OVERRIDE`, `SETUID`, and `KILL` for Gluetun's generated UID-1000 OpenVPN configuration, explicit privilege drop, and supervisor-owned restart | gateway refs unresolved/not ready | Secret rotation restarts/reloads engine per class contract | agent/application mount absence; exact engine capability set; UID/mode read-rewrite, privilege drop, and cross-UID restart; class/status redaction |
| Serve protected gateway DNS | gateway agent + Gluetun | workload DNS is redirected to the gateway overlay on TCP/UDP 1053; the credential-free gateway agent forwards only to Gluetun's loopback TCP/UDP 53 server, which exclusively owns port 53 in the shared network namespace; Pod-local `ndots:1` bounds Kubernetes search expansion; the signed engine image grants unauthenticated access only to DNS-status and public-IP GET routes while all credential-bearing and mutating control routes remain private | DNS and gateway Ready false; enrolled traffic remains denied | level-based listener/rule repair after tunnel DNS observation returns | bind collision, UDP/TCP forwarding, search expansion, route-scoped control authorization, tunnel/DNS loss, no ordinary resolver fallback |
...
| Acquire provider port and program inbound rules | tokenless generic gateway runtime + Gluetun-selected capability | TLS 1.3 mTLS from the exact controller SPIFFE identity; exact gateway UID; capability owns only provider mapping I/O; runtime owns its nftables table and lease state | mapping/rules unknown or false; no successor activation | durable internal-port reservation, atomic rule repair, withdrawal and quarantine | incompatible engine config, capacity regression, restart, expiry, TCP/UDP symmetry, wrong UID/generation |
| Configure a last-resort application adapter | operator-authored immutable WorkloadAdapter | one exact unprivileged Pod, no API/VPN credential, host access, init/ephemeral container, or added capability; exact gateway-runtime mTLS identity | delivery/acknowledgement unknown or false; lease not ready | durable generation state, exact-Pod revalidation, withdraw before handoff | unnecessary adapter selection, wrong Pod/address/generation, restart, stale record, failed withdrawal |
