...
| Mount VPN credential | gateway engine | referenced Secret mounted read-only only into engine; engine drops all capabilities and regains only `NET_ADMIN` plus `CHOWN` for Gluetun's generated OpenVPN configuration | gateway refs unresolved/not ready | Secret rotation restarts/reloads engine per class contract | agent/application mount absence; exact engine capability set; class/status redaction |
