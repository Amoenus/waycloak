# Getting started with Waycloak v0.1.0-rc.7
...
```sh
export WAYCLOAK_TAG=v0.1.0-rc.7
...
`waycloak-0.1.0-rc.7.tgz` and as
`oci://ghcr.io/amoenus/charts/waycloak:0.1.0-rc.7`. Its exact digest is in
...
`oci://ghcr.io/amoenus/waycloak-kcl:0.1.0-rc.7` and as the downloadable
`waycloak-kcl-v0.1.0-rc.7.tar`. Verify its exact digest in `waycloak-kcl.ref`.
Add the OCI module to an existing KCL package, then import its schemas:

```sh
kcl mod add oci://ghcr.io/amoenus/waycloak-kcl --tag 0.1.0-rc.7
```
...
- [Release-candidate scope and limitations](releases/v0.1.0-rc.7.md)
