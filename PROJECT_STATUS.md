...
Phase 2 is not complete. The packaged image still needs to traverse the real admission/allocation startup handshake in a cluster. The next vertical slice imports the test image into the authorized k3s node (or Kind where available), starts an annotated application only after the fake gateway and UID-bound ConfigMap are usable, and proves DNS containment, gateway loss, credential isolation, and annotation removal end to end before any Gluetun integration.
