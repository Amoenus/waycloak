...
An isolated, temporary k3s smoke test has now used an operator-provisioned credential Secret with the pinned Gluetun image. The Secret values were neither read nor printed. Gluetun reached observed readiness without startup permission errors, exposed a syntactically valid public IP different from ordinary cluster egress, and established the minimal explicit engine capability set after dropping all defaults. This is provider evidence, not yet the integrated routed acceptance, so Waycloak still does not claim the `v0.1.0` VPN gateway exit.
...
The production gateway DNS proxy listens on the overlay address for both UDP and TCP, preserves Kubernetes search-domain behavior, sends cluster suffixes to the Pod's observed cluster resolver, and sends all other names to Gluetun's loopback protected resolver. Agent verification now requires HTTP 200 from the manager's `/readyz` endpoint rather than accepting a TCP listener that is serving an unhealthy response. A three-Pod k3s test proves production VXLAN, native forwarding and NAT to a separate target, UDP/TCP and search-domain DNS through the production proxy, idempotent repair, and loss of the dedicated routed target after abrupt gateway deletion with no CNI fallback. The external DNS upstream remains a fixture in this test. The precise next vertical slice is a repeatable integrated Gluetun acceptance through the production gateway and protected Pod path, including public-IP separation, DNS containment, tunnel-loss fail-closed behavior, credential isolation, and observed status transitions.
