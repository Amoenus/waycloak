...
Phase 3 is next. The precise vertical slice is a controller-owned single-replica gateway workload and Service driven by `VPNGateway`, with credentials mounted only into the gateway, a narrow engine interface, observed readiness, and a fake engine acceptance test. Gluetun integration follows that resource-and-health contract; no production VPN security is claimed until the public-egress and tunnel-loss acceptance tests pass.
