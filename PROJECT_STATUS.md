...
This still does not claim a working VPN gateway: no real provider credentials have been used, the routed fixture is explicitly not a VPN, and `DNSReady` and overall `Ready` deliberately remain false pending the real-provider acceptance. `OverlayReady` reflects the gateway manager's observed readiness, which requires provider health and successful native overlay, forwarding, and resolver reconciliation.
