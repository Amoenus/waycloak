...
This still does not claim a working gateway: the fake fixture forwards no internet traffic, no real provider credentials have been used, and `DNSReady` and overall `Ready` deliberately remain false. `OverlayReady` now reflects the gateway manager's observed readiness, which requires both provider health and successful native overlay reconciliation.
...
The gateway manager now reconciles its VXLAN interface and flood-database peers with native netlink operations from that stable desired membership. The interface name is deterministic, conflicting pre-existing attributes fail closed, stale owned peers are removed, and membership changes do not restart the VPN engine. A live k3s test uses the production gateway reconciler and production workload agent on opposite Pods to prove overlay health, then proves abrupt gateway deletion leaves no direct fallback. The test endpoint still supplies only explicit fake DNS and health services. The precise next vertical slice is native gateway forwarding/NAT and DNS service integration behind interfaces, followed by a Gluetun-backed public-egress test using an operator-owned credential Secret.
